...
<section class="order-body">
  <h3 class="order-deal-header">编辑收货地址</h3>
  ...
  <a href="JavaScript:addAddress();" class="btn btn-lg">确认修改</a>
</section>
...
var addressId = Common.GetUrlRequest()['id'];
/**
 * 选择区
 */
 initArea();
...
function getAddressDetail() {
    $.ajax({
        ...
        success: function (res) {
            ...
            $('#consigneePhone').val(res.consigneePhone);
            $('#address').val(res.address);
            $('#areaH').html(res.area);
            $('#areafrom').val(res.areafrom);
        }
    })
}
